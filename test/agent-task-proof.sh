...
# 9, NOT 7. The daemon-down fallback must emit the SAME shape as the daemon path
# (case 2a) — that is the point of a fallback. The agents_tsv contract header
# documents 9 fields, and CLAUDE.md's load-bearing invariant is that all three
# readers demand EXACTLY 9; a 7-field fallback row would misparse. This test was
# authored on d26's branch, whose fallback predated the @fleet_viewer column that
# the viewer work (now on main) added so the fallback can filter viewer panes and
# normalize to the canonical 9. 7 was the stale shape; 9 is the invariant.
[ "$f_nf" = 9 ] && pass 2b || fail 2b "fallback-path \`fleet agents\` emitted ${f_nf:-0} fields, must be 9 (match the daemon path / the 9-field contract)"
